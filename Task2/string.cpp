...
public:
    ...
    string(const char* str)
    {
        ...
        Strncpy(String.get(), str, _capacity);
    }
    string(const string& other)
    {
        ...
        String = make_unique(new char[_capacity]);
        Strncpy(String.get(), other.String.get(), _capacity);
    }
    ...
    string operator+(const string& other) const
    {
        ...
        Strncpy(result.String.get(), String.get(), _size);
        Strncpy(result.String.get() + _size, other.String.get(), other._size + 1);
        return result;
    }
    string& operator+=(const string& other)
    {
    ...
    void reserve(size_t new_capacity)
    {
        if (new_capacity > _capacity)
        {
            ...
            Memcpy(new_string.get(), String.get(), _size + 1);
    ...
    void resize(size_t new_size)
    {
        ...
        {
            reserve(new_size);
            Memset(String.get() + _size, '\0', new_size - _size);
        }
        _size = new_size;
    }
    ...
    string& erase(size_t pos = 0, size_t len = std::string::npos)
    {
        ...
        Memmove(String.get() + pos, String.get() + pos + len, _size - len - pos + 1);
        _size -= len;
        return *this;
    }
    string& insert(size_t pos, const string& new_str)
    {
        ...
        Memmove(String.get() + pos + len, String.get() + pos, _size - pos + 1);
        Memcpy(String.get() + pos, new_str.c_str(), len);
        _size += len;
        return *this;
    }
    int compare(const string& str) const
    {
        return Strcmp(String.get(), str.String.get());
    }
    char& front()
    {
        return String.get()[0];
